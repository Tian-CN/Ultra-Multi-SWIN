...
from model_Tmask_shift_filer_StepMask_SoftMax1_test0505_res_combine_nomask import swin_tiny_patch4_window7_224 as create_model
...
from utils_ConvNeXt import create_lr_scheduler, train_one_epoch, evaluate
...
def main(args):
    ...
    data_transform = {
        "train": transforms.Compose([transforms.RandomResizedCrop(img_size),
                                     transforms.GaussianBlur(kernel_size=int(random.choice(gaussiansize)), sigma=(0.1, 2.0)),
    ...
    #保存历史图
    plt.savefig(
        './data/save_data/tswin_COVID19_SD_20231015_100_random_nomask/plot/butte_scheduler_0.00003_20231015_30_Subject-dependent_dorp_mask_layer_3_Ultra-SWIN-TANSFORMER_all_s' + str(args.all_subj[-1]) + '_val_s' + str(args.val_subj) + '_epochs_' + str(args.epochs) + '.png',
        bbox_inches='tight')
    # plt.show()
    plt.clf()
    # 保存历史数据
    savemat(
        './data/save_data/tswin_COVID19_SD_20231015_100_random_nomask/data/butte_scheduler_0.00003_20231015_30_Subject-dependent_dorp_mask_layer_3_Ultra-SWIN-TANSFORMER_all_s' + str(args.all_subj[-1]) + '_val_s' + str(args.val_subj) + '_epochs_' + str(args.epochs) + '.mat',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_classes', type=int, default=3)   #Ultrasound_photos
    # parser.add_argument('--epochs', type=int, default=10)     #flowers
    parser.add_argument('--epochs', type=int, default=100)       #Ultrasound_photos
    parser.add_argument('--batch-size', type=int, default=32)   #服务器可 64 44 36
    # parser.add_argument('--lr', type=float, default=0.0000001)
    # parser.add_argument('--lr', type=float, default=0.00000001)     # TZK_deform
    parser.add_argument('--lr', type=float, default=0.00001)     # TZK_deform
    ...
    parser.add_argument('--data-path', type=str,
                        default="./data/COVID19")  # Ultrasound_photos text
    # parser.add_argument('--data-path', type=str,
