...
def main(args):
    ...
    #保存历史图
    plt.savefig(
        './data/save_data/swin_origin_20231023_25cross_selected_auto_SI_lr_random/plot/20231023_test_0.0001_Ultra-SWIN-TANSFORMER_all_s' + str(args.all_subj[-1]) + '_val_s' + str(args.val_subj) + '_epochs_' + str(args.epochs) + '.png',
        bbox_inches='tight')
    # plt.show()
    plt.clf()
    # 保存历史数据
    savemat(
        './data/save_data/swin_origin_20231023_25cross_selected_auto_SI_lr_random/data/20231023_test_0.0001_Ultra-SWIN-TANSFORMER_all_s' + str(args.all_subj[-1]) + '_val_s' + str(args.val_subj) + '_epochs_' + str(args.epochs) + '.mat',
        {'history': history, 'subject_set': args.all_subj, 'subject_set_text': args.val_subj})
    print('done')

if __name__ == '__main__':
    for val_subj_ in range(20, 25):
