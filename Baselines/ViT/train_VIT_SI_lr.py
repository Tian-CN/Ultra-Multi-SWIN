...
def main(args):
    ...
    plt.legend()
    #保存历史图
    plt.savefig('./data/save_data/VIT_20231023_25cross_selected_multi_auto_SI_10_lr/plot/20231023_VIT_all_s' + str(args.all_subj[-1]) + '_val_s' + str(args.val_subj) + '_epochs_' + str(args.epochs) + '.png',bbox_inches='tight')
    # plt.show()
    plt.clf()
    # 保存历史数据
    savemat('./data/save_data/VIT_20231023_25cross_selected_multi_auto_SI_10_lr/data/20231023_VIT_all_s' + str(args.all_subj[-1]) + '_val_s' + str(args.val_subj) + '_epochs_' + str(args.epochs) + '.mat',{'history': history, 'subject_set': args.all_subj, 'subject_set_text': args.val_subj})
    print('done')
